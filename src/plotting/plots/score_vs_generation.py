...
def _per_run_curve(
    ...
    """
    Returns list of (generation, score).
    """
...
def plot_score_vs_generation(
    ...
    # If axis provided, ignore 'both' mode — plot only one on the given axis
    if ax is not None:
        if mode == "both":
    ...
    titles = {
        ...
        "best_so_far": "Best Score So Far Over Generations (mean ± SE)",
    }
    ...
    for method, method_runs in by_method.items():
        ...
        for m, ax_ in axes.items():
            ...
            for run in method_runs:
                for gen, score in _per_run_curve(run, m):
            ...
            ax_.plot(gens, means, label=label, linestyle=linestyle)
            ax_.fill_between(gens, means - ses, means + ses, alpha=0.25)
